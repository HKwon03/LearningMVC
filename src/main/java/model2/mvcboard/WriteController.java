...
public class WriteController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// Get 방식 요청 처리
		// List.jsp (view 페이지)에서 글쓰기를 클릭했을 때 글쓰기 뷰페이지 (Write.jsp)
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//1. 파일 업로드 처리 =====================================
			...
			//maxPostSize : 업로드할 최대 용량(web.xml <== 1MB 설정해 놓음)
		...
		//2. 파일 업로드 외 처리 (Form의 변수 값 처리)======================
			...
			if(fileName != null) {	//첨부파일이 비어있지 안다라면
				
				// 새로운 파일이름으로 변경해서 서버에 저장함.(서버의 해당파일이 존재할 경우가 있으므로)
				String now = new SimpleDateFormat("yyyyMMdd_HmsS").format(new Date());
					System.out.println("now : " + now);		//주석 처리
				//확장자만 잘라서 저장
				...
				//DTO에 Setter 주입 (조건 : 파일을 업로드 한 경우에만)
