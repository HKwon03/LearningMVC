...
public class EditController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		// 비밀번호 검증 후 검증이 완료되면 idx 에 해당하는 레코드를 dto에 담아서 Edit.jsp
		// Get방식으로 요청을 받아서 클라이언트가 넘기는 변수를 할당받아서 뷰 페이지로 넘김.
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 1. 파일 업로드 처리 
			// 업로드 디렉토리의 물리적인 경로를 확인해야 한다.(서버에 파일을 업로드할 물리적인 절대경로)
		...
		//2. 파일 업로드 외 처리 ==============================================
			//Request 객체가 아니라 MultipartRequest 객체에서 Form의 변수 값을 받는다.
